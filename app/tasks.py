...
from app.api.models import ProcessConfig, MapperTask, CeleryTaskStatus, \
    Profile, MapperProfile, Status, FileReceiveHistory
from app.api.repositories.common import CRUD
from app.api.repositories.csv import \
...
celery.conf.beat_schedule = {
    'check-csv-files-every-120-seconds': {
        ...
        'schedule': 30.0,
        # TODO: after fixing the seessions issue it should be
        #  returned to be 900.0 nested of 15.0
    },
}
...
def check_csv_files():
    ...
    for task in tasks:
        ...
        if last_run + timedelta(seconds=frequency) < datetime.datetime.now():
            add_tasks.delay(file_id=file_id, file_path=file_path,
                            file_name=file_name, frequency=frequency,
                            process_id=process_id, company_id=company_id,
                            last_run=last_run)
    return "Checked all mappers"


@celery.task(name='add new file to task')
def add_tasks(file_id, file_path, file_name, frequency, process_id, company_id,
              last_run):
    """
    ...
    csv_helper = CSVHelper(task_id=task_id, company_id=company_id,
                           file_id=file_id, file_name=file_name,
                           file_path=file_path, process_id=process_id)
    result = csv_helper.main()
    if result == "No files":
        final_result = f"NO NEW file with prefix: '{file_name}' " \
                       f"with this path '{file_path}'"
    else:
        final_result = f"FILE: '{result['file_name_as_received']}' SENT to " \
...
def mapper_activate(mapper_id: int):
    ...
    if not mapper_task:
        ...
        linked_profile = db.query(MapperProfile).filter(
            MapperProfile.mapper_id == mapper_id).first()
        if linked_profile:
            profile = db.query(Profile).filter(
                Profile.id == linked_profile.profile_id).first()
            if profile.is_deleted:
                return {"mapper": mapper_obj.file_name,
                        "status": "Cant change status, profile is deleted."}
            if not mapper_obj.is_active and not profile.is_active:
                return {"mapper": mapper_obj.file_name,
                        "status": "Cant change status, profile is inactive."}
