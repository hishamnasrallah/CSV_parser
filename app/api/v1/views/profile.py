...
def create_new_profile(request_body: MapperProfile,
                       token=Depends(validate_authorization)):
    data = profile.create_profile(request_body, token)
...
def get_profile_by_id(profile_id: int, db: Session = Depends(CRUD().db_conn),
                      token=Depends(validate_authorization)):
...
@router.put("/profile/{profile_id}/change-status/",
            response_model=MapperProfile)
def change_profile_status(profile_id: int,
                          token=Depends(validate_authorization),
                          db: Session = Depends(CRUD().db_conn)):
    company_id = token["company"]["id"]

    data = profile.change_profile_status(profile_id, company_id, db)
    return http_response(data=data, status=status.HTTP_200_OK,
                         message=ResponseConstants.UPDATED_MSG)
