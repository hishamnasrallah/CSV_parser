...
from app.api.v1.serializers.csv import FileTaskConfigResponse, \
    FileTaskConfigRequest, DashboardResponse, MapperDetailResponse, \
    DebugHistoryResponse, FileTaskResponse
...
def create_new_file_process(request: Request,
                            request_body: FileTaskConfigRequest,
...
def mapper_update(request: Request, id: int,
                  request_body: FileTaskConfigRequest,
                  token=Depends(validate_authorization),
                  db: Session = Depends(CRUD().db_conn)):
...
@router.put("/mappers/{id}/change-status/",
...
def get_mapper_details(request: Request, id: int,
                       token=Depends(validate_authorization),
                       db: Session = Depends(CRUD().db_conn)):
...
def get_mapper_history(request: Request, id: int,
                       token=Depends(validate_authorization),
                       db: Session = Depends(CRUD().db_conn)):
...
def delete_config(request: Request, id: int,
                  token=Depends(validate_authorization),
                  db: Session = Depends(CRUD().db_conn)):
