...
from app.api.models import FileReceiveHistory, ProcessConfig, ProcessMapField, \
    MapperTask, CeleryTaskStatus, MapperProfile, Profile, Status
...
from core.exceptions.csv import CSVConfigDoesNotExist, \
    ...
    ProfileAlreadyDeleted, CantChangeStatusProfileIsInactive, \
    HistoryDoesNotExist
...
def mappers_history(token, file_id, db):
    ...
    """
    configs = db.query(FileReceiveHistory).filter(
        FileReceiveHistory.file_id == file_id).all()
...
def delete_config(id, token, db):
    ...
    :return: nothing
    """
    config = db.query(ProcessConfig).filter(ProcessConfig.id == id,
                                            ProcessConfig.company_id ==
                                            token["company"]["id"])
    if not config.first():
...
def get_company_processes(token, request):
    ...
    :return: the response from the broker that call an api from the
        core application
    """
    company_id = token["company"]["id"]
...
def update_file_history_rows_number_based_on_status(id, status, db):
    history = db.query(FileReceiveHistory).with_for_update().filter(
        FileReceiveHistory.id == id).first()
...
def change_mapper_status(id, token, db):
    ...
    mapper_config_obj = db.query(ProcessConfig).filter(
        ProcessConfig.id == id, ProcessConfig.company_id == company_id).first()
    ...
    try:
        ...
        if not profile_id:
            ...
            if not mapper_profile_obj:
                ...
                link_parser_with_profile_rec = CRUD().add(
                    link_parser_with_profile)
            else:
        ...
        if not request_dict['set_active_at'] and not request_dict["is_active"]:
            ...
            task = mapper_activate.apply_async(args=(mapper_config_obj.id,),
                                               eta=mapper_config_obj.set_active_at.astimezone(
                                                   pytz.utc))
            mapper_task = MapperTask(company_id=int(request_dict["company_id"]),
                                     task_id=str(task),
...
def create_config(request_body, token, db):
    ...
    try:
        ...
        if file_config_rec.set_active_at:
            task = mapper_activate.apply_async(
                args=(file_config_rec.id, ),
...
def execute_mapper(token, id, db):
    ...
    return f"{x} FILE: '{mapper_config_obj.file_name}'" \
           f" SENT to celery \n \n \n \n " + f"  {str(x)}"
